...
namespace AltinnCore.Designer.Controllers
{
    ...
    public class ManualTestingController : Controller
    {
        ...
        [Authorize]
        public async Task<IActionResult> Index(string org, string repo, int partyId, int userId)
        {
            if (userId == 0 || partyId == 0)
            {
                return LocalRedirect($"/designer/{org}/{repo}/ManualTesting/Users/");
            }
            ...
            CheckAndUpdateWorkflowFile(org, repo);
            ...
            StartServiceModel startServiceModel = new StartServiceModel
            {
                ...
                PartyList = _authorization.GetPartyList(requestContext.UserContext.UserId)
                    ...
                    .ToList(),
                PrefillList = _testdata.GetServicePrefill(requestContext.Party.PartyId, org, repo)
            ...
            List<ServiceInstance> formInstances = _testdata.GetFormInstances(requestContext.Party.PartyId, org, repo);
        ...
        public IActionResult Users(string org, string repo)
        {
        ...
        /// <summary>
        /// Redirects the user to the correct url given the repo instance state.
        /// </summary>
        /// <param name="org">Unique identifier of the organisation responsible for the repo.</param>
        /// <param name="repo">Application identifier which is unique within an organisation.</param>
        ...
        /// <returns>The test message box</returns>
        [Authorize]
        public IActionResult RedirectToCorrectState(string org, string repo, Guid instanceId)
        {
            if (HttpContext.Request.Cookies[_generalSettings.GetAltinnPartyCookieName] != null)
            {
                ServiceState currentState = _workflow.GetCurrentState(instanceId, org, repo, Convert.ToInt32(HttpContext.Request.Cookies[_generalSettings.GetAltinnPartyCookieName]));
                string nextUrl = _workflow.GetUrlForCurrentState(instanceId, org, repo, currentState.State);
        ...
        public async Task<IActionResult> LoginTestUser(string org, string repo, int userId)
        {
            ...
            return LocalRedirect($"/designer/{org}/{repo}/ManualTesting/Index?userId={userId}&partyId={profile.PartyId}");
        }
        ...
        /// <param name="org">Unique identifier of the organisation responsible for the repo.</param>
        /// <param name="repo">Application identifier which is unique within an organisation.</param>
        private void CheckAndUpdateWorkflowFile(string org, string repo)
        {
            string workflowFullFilePath = _serviceRepositorySettings.GetWorkflowPath(org, repo, AuthenticationHelper.GetDeveloperUserName(_httpContextAccessor.HttpContext)) + _serviceRepositorySettings.WorkflowFileName;
            ...
            if (!System.IO.File.Exists(workflowFullFilePath))
            {
                ...
                Directory.CreateDirectory(_serviceRepositorySettings.GetWorkflowPath(org, repo, AuthenticationHelper.GetDeveloperUserName(_httpContextAccessor.HttpContext)));
