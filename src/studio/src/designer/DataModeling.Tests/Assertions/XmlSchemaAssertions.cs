...
namespace DataModeling.Tests.Assertions
{
    ...
    public static class XmlSchemaAssertions
    {
        ...
        private static void Equal(XmlSchemaSimpleTypeRestriction expected, XmlSchemaSimpleTypeRestriction actual)
        {
            ...
            List<XmlSchemaFacet> actualFacets = actual.Facets
               .Cast<XmlSchemaFacet>()
               .ToList();

            foreach (XmlSchemaFacet expectedFacet in expected.Facets.Cast<XmlSchemaFacet>())
            {
                bool found = false;
                ...
                if (!found)
                {
                    throw new ContainsException(expectedFacet, actual.Facets);
                }
            }

            if (actualFacets.Count > 0)
            {
                throw new DoesNotContainException(expected.Facets, actualFacets.First());
            }
        }
